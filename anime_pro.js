(function(){
    ...
    function AnimeComponent(){
        ...
        this.load = function(){
            var _this = this;
            // Используем встроенный network.api (Lampac прокси)
            network.api('discover/tv?with_genres=16&with_original_language=ja&language=ru-RU', function(json){
                ...
                Lampa.Loading.stop();
                html.html('<div style="text-align:center; margin-top:50px; color:#fff;">Ошибка сети</div>');
            });
        };
    ...
    // Добавление меню один раз после ready
    Lampa.Listener.follow('menu', function(e){
        if(e.type==='ready'){
            if($('.menu [data-action="anime_final_v42"]').length) return;
            var menuList = $('.menu .menu__list');
            if(!menuList.length) return;

            var menu_item = $(`
                <div class="menu__item selector" data-action="anime_final_v42">
                    <div class="menu__ico">
            ...
            menu_item.on('click', function(){
                Lampa.Activity.push({title:'Аниме Онлайн', component:'anime_final_v42'});
            });
